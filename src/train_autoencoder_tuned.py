...
# Define a deeper and wider 1D convolutional autoencoder
class ConvAutoencoder(nn.Module):
    def __init__(self, num_features, window_size):
        super(ConvAutoencoder, self).__init__()
        self.encoder = nn.Sequential(
            nn.Conv1d(num_features, 32, kernel_size=3, stride=2, padding=1),
            nn.ReLU(True),
        ...
        self.decoder = nn.Sequential(
            ...
            nn.ReLU(True),
            nn.ConvTranspose1d(32, num_features, kernel_size=3, stride=2, padding=1, output_padding=1),
            nn.Sigmoid()
        )
...
model_path = 'src/conv_autoencoder_tuned.pth'
